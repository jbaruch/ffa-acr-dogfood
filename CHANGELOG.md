...
- Deliver the failure report instead of the failure status. The hook exits 0 once the payload is written, carrying the update's exact exit code, its output and the recovery step inside the payload and on stderr. Codex 0.153.2 parses a SessionStart hook's stdout only on exit 0 — `parse_completed` in `codex-rs/hooks/src/events/session_start.rs` at `rust-v0.153.2` (commit `657a993cbee87acf52d14b758ce49dbd46d1b8eb`) records any other status as `hook exited with code N` and appends no model context, and Cursor fails open on a non-zero exit the same way. Returning the update's own status therefore hid the very failure the hook exists to report: it reached the terminal and never reached the agent. The optional update is best-effort at this boundary, so its status travels inside the report; a known failure never reads to the agent as a success, and a payload the hook cannot write still exits non-zero with its own diagnostic.
