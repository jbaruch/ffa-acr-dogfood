...
- Skip the optional SessionStart Tessl update with an actionable notice when Tessl is absent; report update failures when it is present. Cover both paths and safe sourcing in the package gate.
- Report that SessionStart outcome to the agent, not just to the terminal: the skip, a failure, and a successful update that printed something each emit a `Session-start status — ` context payload in the shape the running agent reads.
- Deliver the failure report instead of the failure status. The hook exits 0 once the payload is written, carrying the update's exact exit code, both of its output streams and the recovery step inside the payload and on stderr. Codex 0.153.2 parses a SessionStart hook's stdout only on exit 0 — `parse_completed` in `codex-rs/hooks/src/events/session_start.rs` at `rust-v0.153.2` (commit `657a993cbee87acf52d14b758ce49dbd46d1b8eb`) records any other status as `hook exited with code N` and appends no model context, and Cursor fails open on a non-zero exit the same way. Returning the update's own status therefore hid the very failure the hook exists to report: it reached the terminal and never reached the agent. The optional update is best-effort at this boundary, so its status travels inside the report; a known failure never reads to the agent as a success, and a payload the hook cannot write still exits non-zero with its own diagnostic. Both of the update's streams are captured, separately and by name, because `tessl update` diagnoses some failures on stderr alone: a report carrying stdout alone handed the agent an exit code and a retry command with no error to act on, which was the one thing it needed. What the update wrote on stderr still reaches the terminal, where it would have appeared uncaptured.
